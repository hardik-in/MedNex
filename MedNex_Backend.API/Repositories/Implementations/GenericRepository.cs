...
namespace MedNex_Backend.API.Repositories.Implementations
{
    public class GenericRepository<T> : IGenericRepository<T> where T : BaseEntity
    {
        ...
        {
            return await _dbSet
                .Where(e => e.PublicId == publicId && !e.IsDeleted)
                .FirstOrDefaultAsync();
        }
            ...
            Expression<Func<T, bool>>? filter = null)
        {
            ...
            if (!string.IsNullOrWhiteSpace(request.Search))
                query = query.Where(e => e.ReferenceId != null &&
                    e.ReferenceId.Contains(request.Search));
        ...
        public async Task<int> GetYearlyCountAsync(int year)
        {
